...
- **No test was written for the two visual/animation-heavy landing-page components** (`hero-demo.tsx`, `pipeline-flow.tsx`) - they're pure presentation with no business logic. They previously failed ESLint's `react-hooks/set-state-in-effect` rule; that's since been fixed (see the `fix: resolve eslint errors and warnings` commit), but they still don't have dedicated tests.
