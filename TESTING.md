...
- **The AI agent route itself (`app/api/agent/route.ts`) has no dedicated automated test.** `lib/agent-tools.ts` (what the agent can call) is unit tested, and `lib/mcp.ts`'s tenant-scoping is exercised indirectly through the `lib/tenant.ts` tests it's built on - but the route handler's own logic (streaming a response through the Vercel AI SDK, live tool-calling against a real Anthropic model, and the Sanity Context MCP connection) would need either a live Anthropic API key and a live MCP connection in CI, or a fairly involved mock of the AI SDK's streaming protocol. Neither felt like a good trade for the value versus the unit tests already covering the security-relevant logic underneath it (tenant scoping, tool error handling).
